...
_move_actions = [
    PositionActions(_MOVE_SCREEN.id, f"MOVE_TO_{chr(i)}")
    for i in range(ord("A"), ord("A") + _GRID_RESIZE_FACTOR**2)
]

_build_barracks_positions = [
    PositionActions(_BUILD_BARRACKS.id, f"BUILD_BARRACKS_AT_{chr(i)}")
    for i in range(ord("A"), ord("A") + _GRID_RESIZE_FACTOR**2)
]

_build_supply_positions = [
    PositionActions(_BUILD_SUPPLY_DEPOT.id, f"BUILD_SUPPLY_DEPOT_AT_{chr(i)}")
    for i in range(ord("A"), ord("A") + _GRID_RESIZE_FACTOR**2)
]
