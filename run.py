...
with open(os.path.join(os.path.dirname(__file__), "config", "avail_maps.yaml"), "r") as fp:
...
def run_loop():
    ...
    episodes = range(_EPISODES+_NUM_EVALS*_EVAL_WINDOW) if not _EVAL_MODE else range(_EPISODES)
    train_episodes = 0
    eval_episodes = 0
    episode = 0
    ...
    while current_try < num_tries:
        ...
        try:
            ...
            while episode < len(episodes):
                ...
                if (train_episodes == eps_to_start_test) or (test_mode and eval_episodes < _EVAL_WINDOW):
                ...
                while not done:
                    if actions_taken % _STEPS_TO_CHANGE_STRATEGY == 0:
                        ...
                        if not (_EVAL_MODE or test_mode) and actions_taken > 0:
                            ...
                            if upper_loss is not None:
                                upper_losses.append(upper_loss.item())       # from tensor to float
                    ...
                    micro_action, micro_action_index = agent.choose_micro_action(state, available_actions, feature_units)
                    micro_action_index = torch.tensor([[micro_action_index]], dtype=torch.long)     # change from int to tensor
                    # print(micro_action)
                    ...
                    if not done:
                        won = agent.check_win(feature_units=next_feature_units) if _MAP_NAME != 'FindAndDefeatZerglings' else agent.check_win(units_defeated=units_defeated)

                    # TRAIN LOWER AGENT
                    if not (_EVAL_MODE or test_mode):
                        agent.update_memory(states_pair[0], micro_action_index, states_pair[1], torch.tensor([reward]), done, lower=True)
                    ...
                    if won:
                        ...
                        ep_results.append(1) if not (_EVAL_MODE or test_mode) else eval_ep_results.append(1)
                        units_defeated = 0 
                ...
                if test_mode or _EVAL_MODE:
                    eval_rewards.append(cum_reward.item())     # converting from np.int32 to int
                    print(f'Episode reward: {cum_reward}')
                    eval_ep_results.append(get_result(obs.observation, _MAP_NAME, units_defeated))
                else:
                    rewards.append(cum_reward.item())     # converting from np.int32 to int
                    print(f'Episode reward: {cum_reward}')
                    ep_results.append(get_result(obs.observation, _MAP_NAME, units_defeated))
